...
class Lex:
    ...
    def assign_token(self):
        ...
        if self.char == '=':
            self.recognised_string += self.char
            return token.Token(self.recognised_string, 'relOP', self.current_line)
        else:
            self.file.seek(self.position-1)
            return token.Token(self.recognised_string,'asgn',self.current_line)
    ...
    def delimeter_token(self):
        ...
        return token.Token (self.recognised_string, 'del', self.current_line)
        ...
        if self.char == '$':
            ...
            while self.state == 'comment':
                ...
                if self.char == '#':
                    ...
                    if self.char == '$':
                        self.recognised_string += self.char
                        return token.Token(self.recognised_string, 'comments', self.current_line)
                    self.file.seek(self.position)
        elif self.char in self.grouping_symbols:
            self.grouping_symbol_token()
            return token.Token(self.recognised_string, 'grouping', self.current_line)
        elif self.char in self.alphabet:
            self.keyword_token()
            return token.Token(self.recognised_string, 'declare', self.current_line)

    def grouping_symbol_token(self):
        # self.state = 'grouping_symbol'
        self.recognised_string += self.char
        return token.Token(self.recognised_string, "grouping_symbol", self.current_line)
        ...
        while self.state == 'keyword':
            ...
            if (self.char not in self.alphabet and self.char not in self.digits) or self.char == '' or self.char == ' ':
                ...
                if self.recognised_string in self.keywords:
                    self.file.seek(self.position - 1)
                    return token.Token(self.recognised_string, "keyword", self.current_line)
                self.file.seek(self.position - 1)
                return token.Token(self.recognised_string, "var", self.current_line)
    ...
    def digit_token(self):
        ...
        while self.state == 'digit':
            self.char = self.file.read(1)
            self.position = self.file.tell()
            if self.char not in self.digits or self.char == '' or self.char == ' ' or self.char in self.grouping_symbols:
                self.state = 'terminal'
